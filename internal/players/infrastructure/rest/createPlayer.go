...
func createPlayerHttpAdapter(body io.Reader) (createPlayer, error) {
	...
	if err := dec.Decode(&playerJson); err != nil {
		...
		case errors.As(err, &ute):
			return createPlayer{}, fmt.Errorf(
				"invalid type for field %q: got %q, expected %s",
				ute.Field, ute.Value, ute.Type.String(),
			)
		case errors.As(err, &se):
			return createPlayer{}, fmt.Errorf("invalid JSON syntax at byte %d", se.Offset)
		case strings.HasPrefix(err.Error(), "json: unkown field"):
			field := strings.TrimPrefix(err.Error(), "json: unknown field ")
			field = strings.Trim(field, "\"")
			return createPlayer{}, fmt.Errorf("unknown field: %s", field)
		default:
			if strings.Contains(err.Error(), "unknown field") {
				return createPlayer{}, fmt.Errorf("%v", err)
			}
			return createPlayer{}, fmt.Errorf("invalid JSON: %v", err)
		}
	}
